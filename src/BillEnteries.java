...
public class BillEnteries  {
	
	Stage window;
	 
	static Scene scene;
	
	BillEnteries(String title,int width,int height){
		window = new Stage();
	...
	int totalLines(){
		try {
			...
			FileReader fr = new FileReader("file://../data/Bill Enteries.txt.txt");
	...
	String[][] Reader() {
	 ...
	 public static void Writer(String data, boolean append) {
		 ...
		 try {
			 FileWriter fw = new FileWriter("file://../data/Bill Enteries.txt", append);
			 fw.write(data);
			 fw.close();
