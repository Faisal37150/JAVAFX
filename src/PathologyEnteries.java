...
public class PathologyEnteries  {
	
	Stage window;
	 
	static Scene scene;
	
	PathologyEnteries(String title,int width,int height){
		window = new Stage();
		...
		window.setHeight(height);
			...
			Scene scene = new Scene(root,650,600,Color.MEDIUMPURPLE);
	...
	int totalLines(){
		try {
			...
			FileReader fr = new FileReader("file://../data/PathologyEnteries.txt");
	...
	String[][] Reader() {
	 ...
	 public static void Writer(String data, boolean append) {
		 ...
		 try {
			 FileWriter fw = new FileWriter("file://../data/PathologyEnteries", append);
			 fw.write(data);
			 fw.close();
